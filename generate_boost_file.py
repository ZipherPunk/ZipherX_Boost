...
"""
    ...
    result = subprocess.run(
        ["gh", "release", "view", tag, "--repo", "ZipherPunk/ZipherX_Boost"],
        cwd=repo_dir,
        capture_output=True,
        text=True
    )
    if result.returncode == 0:
        ...
        subprocess.run(
            ["gh", "release", "delete", tag, "--yes", "--repo", "ZipherPunk/ZipherX_Boost"],
            cwd=repo_dir,
            capture_output=True,
            text=True
        )
        ...
        subprocess.run(
            ["git", "push", "zipherpunk", f":refs/tags/{tag}"],
            cwd=repo_dir,
            capture_output=True,
            text=True
        )
...
def release_only(source_dir: str, repo_dir: str):
    """Just git push + GitHub release from existing files (no regeneration)."""
    log("=" * 70)
    log("RELEASE-ONLY MODE — Pushing existing files to GitHub")
    log("=" * 70)

    # Read manifest
    manifest_path = os.path.join(source_dir, "zipherx_boost_manifest.json")
    if not os.path.exists(manifest_path):
        manifest_path = os.path.join(repo_dir, "zipherx_boost_manifest.json")
    if not os.path.exists(manifest_path):
        log_error(f"Manifest not found in {source_dir} or {repo_dir}")
        sys.exit(1)

    with open(manifest_path) as f:
        manifest = json.load(f)

    chain_height = manifest["chain_height"]
    output_count = manifest.get("output_count", 0)
    spend_count = manifest.get("spend_count", 0)
    tree_root = manifest.get("tree_root", "")
    log(f"Chain height from manifest: {chain_height}")
    log(f"Outputs: {output_count}, Spends: {spend_count}")

    # Get file size
    boost_file = os.path.join(source_dir, "zipherx_boost_v1.bin")
    file_size = os.path.getsize(boost_file) if os.path.exists(boost_file) else 0

    # Copy files to repo
    copy_files_to_repo(source_dir, repo_dir)

    # Update SHA256SUMS
    update_sha256sums(source_dir, repo_dir)

    # Update README.md
    block_hash = manifest.get("block_hash", "0" * 64)
    sha256 = manifest.get("files", {}).get("uncompressed", {}).get("sha256", "0" * 64)
    compressed_size = manifest.get("files", {}).get("compressed", {}).get("size", None)
    update_readme(
        repo_dir=repo_dir,
        chain_height=chain_height,
    ...
    if not git_success:
        log("  ⚠️  Git push failed, continuing with release...")
...
def main():
    ...
    parser.add_argument('--release-only', action='store_true',
                        help='Skip generation — just git push + GitHub release from existing files')

    args = parser.parse_args()
    output_dir = args.output_dir
    repo_dir = args.repo_dir

    # --release-only: skip generation, just push + release from existing files
    if args.release_only:
        release_only(output_dir, repo_dir)
        return
